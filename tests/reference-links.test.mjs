...
const candidates = {
  references: [
    { handle: 'ref-1', printed_label: '1.', text: 'Example reference.' },
    { handle: 'ref-2', printed_label: '2.', text: 'Unmentioned reference.' }
  ],
  citation_mentions: [
...
const links = {
  citation_decisions: [{ citation_handle: 'cite-1', classification: 'bibliographic_citation', reference_handles: ['ref-1'] }]
};

assert.equal(hasReferenceLinkCandidates(candidates), true);
assert.equal(referenceLinksFormat(candidates).json_schema.name, 'deskreview_reference_relation_decisions_v4');
assert.deepEqual(referenceLinksFormat(candidates).json_schema.schema.properties.citation_decisions.items.properties.citation_handle.enum, ['cite-1']);
...
assert.equal(hasValidReferenceLinks({ citation_decisions: [{ citation_handle: 'cite-1', classification: 'not_bibliographic', reference_handles: ['ref-1'] }] }, candidates), false);
assert.equal(hasValidReferenceLinks({ citation_decisions: [{ citation_handle: 'cite-1', classification: 'bibliographic_citation', reference_handles: [] }] }, candidates), false);
...
assert.equal(applied[0].body_occurrences[0].source_alignment, 'exact');
assert.equal(applied[1].body_occurrences.length, 0);
const excluded = applyReferenceLinks([
  { link_handle: 'ref-1', text: 'Example reference.' },
  { link_handle: 'ref-2', text: 'Unmentioned reference.' }
], candidates, { citation_decisions: [{ citation_handle: 'cite-1', classification: 'not_bibliographic', reference_handles: [] }] });
assert.equal(excluded[0].body_occurrences.length, 0);
