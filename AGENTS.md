...
- User-approved citation-anchor exception: presentation-only alignment may ignore punctuation, citation delimiters, whitespace, case, and hyphen/dash separators inside the single model-declared raw OCR block. It may proceed only when the returned word-and-number token sequence has exactly one contiguous match in that block; the literal matched OCR substring becomes the HTML anchor. It must preserve token order, words, numbers, and diacritics, and must never search another block, change a reference relationship, infer a missing citation, split or merge a citation group, or accept an ambiguous match.
...
- [docs/reference-inventory-v2-decision.md](docs/reference-inventory-v2-decision.md) supersedes the retired Reference Audit V2 path. Its production request owns bibliography inventory only and requires one explicit result per supplied OCR bibliography block. The broad annotation contract owns exact body citation occurrences, and the bounded reference-relation stage may map only already validated citation and bibliography handles.
